...
namespace aru_software_eng_UI
{
	 ...
	 * 
	 * This is now a deprecated class that should be deleted in time for the final merge.
	 * It serves, for now, as a reference point for the old backend code.
	 * 
	 * -[ John | SID: 2118490 ]
	*/
	public class DatabaseWrapperDefunct
	{
		...
		private DatabaseWrapperDefunct()
		{
			connection_to_database = new SqlConnection(Properties.Settings.Default.database_connection_string);
		}

		static DatabaseWrapperDefunct singleton_instance;
		//singleton design pattern, done so as to prevent (bad) redundant copies.
		//	-JE oct-26.0
		public static DatabaseWrapperDefunct getDatabaseWrapperInstance()
		{
			if(singleton_instance == null)
			{
				singleton_instance = new DatabaseWrapperDefunct();
			}
